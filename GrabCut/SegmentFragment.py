...
binthresh=BINARIZATION_THRESHOLD
fill_holes=1
...
#list_to_process = '/Volumes/Maxtor/DSS/files_from_nli_06112018.txt'
#list_to_process = '/Volumes/Maxtor/DSS/files_from_nli_27112018_third_batch.txt'  # file with .txt extension
list_to_process = '/Volumes/Maxtor/DSS/files_from_nli_fourth_batch.txt'
#list_to_process = '/Volumes/Maxtor/DSS/test_1112-1-Fg002-V.txt'
logf = open(list_to_process[0:-4]+ '_error.txt', 'w')

base_path = '/Volumes/Maxtor/DSS/'
#img_path = base_path + 'MR/'
#img_path = '/Volumes/Seagate Expansion Drive/Third batch SQE 141018/'
img_path = '/Volumes/Seagate Expansion Drive/2011to2013/'
#img_path = '/Volumes/My Passport/Second_batch_SQE_060618/'
...
for j in range(2901,3100):#len(onlycolorimgs)):
#for j in range(0, 1):
    ...
    try:
     ...
     #       imgcolorname = os.path.join(img_path,plate_num,img_name)
            imgcolorname = os.path.join(img_path,img_parent_path, img_name)
...
#        im_out = imbinary | im_floodfill_inv  #kernel = np.ones((5, 5), np.uint8)
    ...
    #    print (stats[cc_ind])
        ...
        if fill_holes:
           ...
           # holes_mask1 = np.zeros(imbinary.shape, dtype=np.uint8)
            holes_mask1 = np.where((holes_mask == 255),1,0)
            ccmask = ccmask + holes_mask1
            #outimgname = img_name[0:-4] + '_gc_holes_flood_fill.png'
            #cv2.imwrite(os.path.join(debug_path, plate_num, outimgname), img_floodfill * 255)
            #outimgname = img_name[0:-4] + '_gc_holes_mask1.png'
            #cv2.imwrite(os.path.join(debug_path, plate_num, outimgname), holes_mask1 * 255 )
    ...
    # Crop Connected Component
        ...
        if not os.path.isdir(os.path.join(frag_cords,plate_num)):
            ...
            #print('mkdir '+os.path.join(frag_cords,plate_num).replace(" ", "\ "))
            os.system('mkdir '+os.path.join(frag_cords,plate_num).replace(" ", "\ "))
       ...
       # cv2.imshow("cropped",crop_img)
        ...
        # ---- Connected component
        # You need to choose 4 or 8 for connectivity type
        print('Find Connected Components on GrabCut mask')
        connectivity = 4
        # Perform the operation
    ...
    # apply CC mask
        ...
        if not os.path.isdir(os.path.join(frag_path,plate_num)):
            ...
            os.system('mkdir ' + os.path.join(frag_path,plate_num).replace(" ", "\ "))
